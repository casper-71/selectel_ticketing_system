...
def _history_mapper(local_mapper):
    ...
    if (
        not super_mapper or local_mapper.local_table is not super_mapper.local_table
    ):
        cols = []
        ...
        for column in local_mapper.local_table.c:
            ...
            if (
                len(orig_prop.columns) > 1 or orig_prop.columns[0].key != orig_prop.key
            ):
...
def create_version(obj, session, deleted=False):
    ...
    if not obj_changed:
        ...
        for prop in obj_mapper.iterate_properties:
            if (
                isinstance(prop, RelationshipProperty) and attributes.get_history(
